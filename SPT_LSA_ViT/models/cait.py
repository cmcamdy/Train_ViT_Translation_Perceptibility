        ...
        if not self.is_LSA:
            ...
            dots = torch.mul(
                ...
                scale.unsqueeze(0).unsqueeze(-1).unsqueeze(-1).expand(
                    (x.size(0), self.heads, 1, 1)))
            if self.if_patch_attn:
                dots[:, :, self.mask[:, 0], self.mask[:, 1]] = -1e-9

        dots = einsum('b h i j, h g -> b g i j', dots,
                      self.mix_heads_pre_attn)  # talking heads, pre-softmax
        attn = self.attend(dots)
        attn = einsum('b h i j, h g -> b g i j', attn,
                      self.mix_heads_post_attn)  # talking heads, post-softmax
...
class CaiT(nn.Module):
    def __init__(self,
        ...
        self.patch_transformer = Transformer(dim,
                                             num_patches,
                                             depth,
                                             heads,
                                             dim_head,
                                             ...
                                             if_patch_attn=True,
                                             is_LSA=is_LSA)
        self.cls_transformer = Transformer(dim,
                                           num_patches,
                                           cls_depth,
                                           heads,
                                           dim_head,
