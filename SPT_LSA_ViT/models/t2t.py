...
class Attention(nn.Module):
    ...
    def forward(self, x):
        ...
        qkv = self.qkv(x).reshape(B, N, 3, self.num_heads,
                                  self.in_dim // self.num_heads).permute(
                                      2, 0, 3, 1, 4)
        ...
        x = v.squeeze(
            1
        ) + x if self.num_heads == 1 else x + residual  # because the original x has different size with current x, use v to do skip connection
...
class T2T_module(nn.Module):
    ...
    def __init__(self,
        ...
        if img_size == 64:
            ...
            self.soft_split0 = nn.Unfold(kernel_size=(3, 3),
                                         stride=(2, 2),
                                         padding=(1, 1))
            ...
            self.attention1 = Token_transformer(dim=in_chans * 3 * 3,
                                                in_dim=token_dim,
                                                num_heads=1,
                                                mlp_ratio=1.0,
            ...
            self.attention2 = Token_transformer(dim=token_dim * 3 * 3,
                                                ...
                                                num_patches=self.num_patches,
                                                is_LSA=is_LSA)
            self.num_patches = (img_size // (2 * 2 * 2)) * (img_size //
                                                            (2 * 2 * 2))
