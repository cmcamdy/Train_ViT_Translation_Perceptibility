...
class WindowAttention(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        if not self.is_LSA:
            ...
            q = torch.mul(
                q,
                scale.unsqueeze(0).unsqueeze(-1).unsqueeze(-1).expand(
                    (B_, self.num_heads, 1, 1)))
    ...
    def __init__(self,
        ...
        if self.shift_size > 0:
            ...
            mask_windows = window_partition(
                img_mask,
                self.window_size)  # N_w^2, window_size, window_size, 1
            mask_windows = mask_windows.view(
                -1, self.window_size *
                self.window_size)  # N_w^2, window_size, window_size
            attn_mask = mask_windows.unsqueeze(1) - mask_windows.unsqueeze(
                2
            )  # (N_w^2, 1, window_size, window_size) - (N_w^2, window_size, 1, window_size)
    ...
    def __init__(self,
        ...
        self.blocks = nn.ModuleList([
            SwinTransformerBlock(dim=dim,
                                 ...
                                 is_LSA=is_LSA) for i in range(depth)
        ])
...
class SwinTransformer(nn.Module):
    ...
    def __init__(self,
        ...
        if not is_SPT:
            ...
            self.patch_embed = ShiftedPatchTokenization(3,
                                                        embed_dim,
                                                        patch_size,
                                                        is_pe=True)
            self.img_resolution = (img_size // patch_size,
                                   img_size // patch_size)

        # absolute position embedding
        self.absolute_pos_embed = nn.Parameter(
            torch.zeros(1, self.img_resolution[0]**2, embed_dim))
        ...
        self.img_resolution = [
            self.img_resolution[0] // (2**(self.num_layers - 1)),
    ...
    def forward(self, x):
        x, x_region = self.forward_features(x)
        x = self.head(x)
        return x, x_region
