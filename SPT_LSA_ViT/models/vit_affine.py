...
class ViT(nn.Module):
    def __init__(self,
        ...
        self.num_patches = (image_height // patch_height) * (image_width //
                                                             patch_width)
        ...
        if not is_SPT:
            self.to_patch_embedding = nn.Sequential(
                Rearrange('b c (h p1) (w p2) -> b (h w) (p1 p2 c)',
                          p1=patch_height,
                          p2=patch_width), nn.Linear(self.patch_dim, self.dim))

        else:
            self.to_patch_embedding = ShiftedPatchTokenization(3,
                                                               self.dim,
                                                               patch_size,
                                                               is_pe=True)
        ...
        self.mlp_head = nn.Sequential(nn.LayerNorm(self.dim),
