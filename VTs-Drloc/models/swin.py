...
class SwinTransformerBlock(nn.Module):
    ...
    def forward_with_attention(self, x):
        ...
        # W-MSA/SW-MSA
        attn_windows, attn = self.attn.forward_with_attention(
            x_windows, mask=self.attn_mask)  # nW*B, window_size*window_size, C
        ...
        if self.use_drloc:
            ...
            if self.use_multiscale:
                for i_layer in range(self.num_layers):
                    self.drloc.append(
                        DenseRelativeLoc(
                            in_dim=min(int(embed_dim * 2**(i_layer + 1)),
                                       self.num_features),
                            out_dim=2 if drloc_mode == "l1" else max(
                                img_size // (4 * 2**i_layer), img_size //
                                (4 * 2**(self.num_layers - 1))),
                ...
                self.drloc.append(
                    DenseRelativeLoc(
                        ...
                        out_dim=2 if drloc_mode == "l1" else img_size //
                        (4 * 2**(self.num_layers - 1)),
                        sample_size=sample_size,
                        drloc_mode=drloc_mode,
                        use_abs=use_abs))
