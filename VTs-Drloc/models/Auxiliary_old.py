...
class Auxillary(nn.Module):
    def __init__(self, sub_model, dim, out_cls=1, with_scale=True, with_trans=True):
        ...
        # if self.with_trans:
        #     self.mlp_head_trans = nn.Sequential(nn.LayerNorm(dim*2), nn.Linear(dim*2, out_cls))
        # if self.with_scale:
        #     self.mlp_head_scale = nn.Sequential(nn.LayerNorm(dim*2), nn.Linear(dim*2, out_cls))
