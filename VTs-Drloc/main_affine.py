...
from models import build_modelv2
    ...
    for idx, (samples, targets) in enumerate(data_loader):
        ...
        if config.TRAIN.SCALE and config.TRAIN.TRANS:
            outputs, outputs_translated, trans_dis, scale_dis = model(
                samples, samples_translated)
        if config.TRAIN.SCALE and not config.TRAIN.TRANS:
            outputs, outputs_translated, scale_dis = model(
                samples, samples_translated)
        if not config.TRAIN.SCALE and config.TRAIN.TRANS:
            outputs, outputs_translated, trans_dis = model(
                samples, samples_translated)
        # outputs, outputs_translated, trans_dis = model(samples, samples_translated)
        # import pdb; pdb.set_trace()

        if config.TRAIN.ACCUMULATION_STEPS > 1:
            loss_oral = criterion_sup(outputs.sup, targets)
            loss_translated = criterion_sup(outputs_translated.sup, targets)
            # loss_trans_mse = criterion_trans(trans_dis, targets_translate) * lambda_drloc * 2
            if config.TRAIN.SCALE and config.TRAIN.TRANS:
                ...
                loss = loss_oral + loss_translated + loss_trans_mse + loss_scale_mse
            if config.TRAIN.SCALE and not config.TRAIN.TRANS:
                loss_scale_mse = criterion_scale(
                    scale_dis, targets_translate) * lambda_drloc * 2
                loss = loss_oral + loss_translated + loss_scale_mse
            if not config.TRAIN.SCALE and config.TRAIN.TRANS:
                loss_trans_mse = criterion_trans(
                    trans_dis, targets_translate) * lambda_drloc * 2
