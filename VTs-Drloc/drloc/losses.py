...
def variance_aware_regression(pred, beta, target, labels, lambda_var=0.001):
    ...
    return torch.mean(
        torch.exp(-pred_log_var) * squared_error + lambda_var * pred_log_var)
...
def relative_constraint_cbr(deltaxy, predxy, loss_type="regression_mse_var"):
    ...
    # targetx, targety = deltaxy[:,0], deltaxy[:,1]   # [N, ], [N, ]
    targetx, targety = deltaxy[:, 0].long(), deltaxy[:,
                                                     1].long()  # [N, ], [N, ]
    betax, betay = F.softmax(predx, dim=1), F.softmax(predy,
                                                      dim=1)  # [N, C], [N, C]
    ...
    if loss_type in ["regression_mse", "regression_mse_var"]:
        if "var" in loss_type:
            # Variance aware regression.
            lossx = variance_aware_regression(pred_idx, betax, true_idx,
                                              labels)
            lossy = variance_aware_regression(pred_idy, betay, true_idy,
                                              labels)
        else:
        ...
        raise NotImplementedError(
            "We only support regression_mse and regression_mse_var now.")
...
def cal_selfsupervised_loss(outs, args, lambda_drloc=0.0):
    ...
    if args.TRAIN.USE_DRLOC:
        ...
        loss_drloc = 0.0
        for deltaxy, drloc, plane_size in zip(outs.deltaxy, outs.drloc,
                                              outs.plz):
...
def relative_constraint_cbr_spt(predxy,
                                deltaxy,
                                loss_type="regression_mse_var"):
    predx, predy = predxy[:, 0].unsqueeze(1), predxy[:, 1].unsqueeze(1)
    # import pdb
    # pdb.set_trace()
    num_classes = predx.size(0)
    targetx, targety = deltaxy[:, 0], deltaxy[:, 1]  # [N, ], [N, ]
...
if __name__ == "__main__":
    ...
    # trans
    print(relative_constraint_cbr_spt(input1, target))
    input1 = torch.randn(2, 1)
    target = torch.randn(2, 1)
    # scale
    print(relative_constraint_l1(input1, target))
